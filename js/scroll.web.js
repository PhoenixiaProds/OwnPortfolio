...
******                   Table of Content                   ******
******                   ----------------                   ******
******  - Mouse Wheel   jScrollPane v2.0.0                  ******
*****************************************************************/
...
/* Registering module and creating function */
(function(b,a,c){
...
/* Initializing function and html integrators */            
            function at(aR){
...
/* Temporarily overriding CSS positioning of container and pane to measure dimensions accurately */ 
                else {
                    D.css("width","");
...
/* In cas enothing changed since last check */ 
...
/* Initializing function depending on sizes detected */ 
...
/* Initializing vertical scrolling function with html elements */ 
            function aG(){
                if(aA){
...
/* Making pane thinner to allow for vertical scrollbar */ 
            function o(){
                aq.height(t+"px");
                I=0;
                ...
                Y.width(ak-X-f);
                try{
/* Adding margin to position scrollbar on left or right sides depending on css properties */ 
                    if(U.position().left===0){Y.css("margin-left",X+"px")
                }
            }
            catch(s){}
        }
/* Initializing horizontal scrolling function with html elements */ 
        function z(){
            if(aF){
...
/* Initializing scrollbars resizing */ 
        function F(){
            if(aF&&aA){
...
/* Setting up default arrows appearing and fix for Mac users */ 
...
/* Preventing JSP from being scrolled */ 
        function V(s,aJ){
            if(!aA){
                return
            }
...
/* Can't just check if animate because false also is a value that could be passed in  */ 
            if(aJ){
...
/* Preventing again JSP from being scrolled */ 
        function W(aJ,s){
            if(!aF){
                return
            }
...
/* Catching up errors from the lookup to prevent from illegal jQuery selectors */ 
            try{
                aO=b(aW)
            }
            catch(aN){
            ...
            am.scrollTop(0);
            am.scrollLeft(0);
/* Looping through parents adding offset top of elements positionned relatively between focused element ans pane */ 
            while(!aO.is(".jspPane")){
                s+=aO.position().top;
                aV+=aO.position().left;
                aO=aO.offsetParent();
                if(/^body|html$/i.test(aO[0].nodeName)){
/* Quitting because of ending up too high on document */
                    return
                }
            }
            aJ=aB();
...
/* Canceling the repeat key for FF or OSX if the JSP scrolls */
...
/* Waiting until container's scroll messes up while hash does its job */
...
/* Registering link handler once */
        function ai(){
            b("a.jspHijack").unbind("click.jsp-hijack").removeClass("jspHijack")
        }
/* Remembering handler was bound and using live handler to capture newly created links */
        function m(){
            ai();
            b("a[href^=#]").addClass("jspHijack").bind("click.jsp-hijack",function(){
/* Checking if link points to same page and allowing to use # tag to point to a chapte ron the same url page */
...
/* Initializing touch on mobile devices */
        function ao(){
...
/* Reinitializing the scroll pane through settings override */
            b.extend(Q,{
                reinitialise:function(aJ){
                    aJ=b.extend({},az,aJ);
                    at(aJ)
                },
/* Scrolling to the specified element into view */
                scrollToElement:function(aK,aJ,s){
                    ab(aK,aJ,s)
                },
/* Scrolling the pane to the specified co-ordinates */
                scrollTo:function(aK,s,aJ){
                    N(aK,aJ);
                    M(s,aJ)
                },
/* Scrolling the pane to the specified co-ordinates at the left of the viewport */
                scrollToX:function(aJ,s){
                    N(aJ,s)
                },
/* Scrolling the pane to the specified co-ordinates at the top of the viewport */
                scrollToY:function(s,aJ){
                    M(s,aJ)
                },
/* Scrolling the pane to the specified percentage of its maximum horizontal scroll position */
                scrollToPercentX:function(aJ,s){
                    N(aJ*(T-ak),s)
                },
/* Scrolling the pane to the specified percentage of its maximum vertical scroll position */
                scrollToPercentY:function(aJ,s){
                    M(aJ*(Z-v),s)
                },
/* Scrolling the pane by the specified amount of pixels */
...
/* Positioning horizontal dragbar at the specified position */
                positionDragX:function(s,aJ){
                    W(s,aJ)
                },
/* Positioning vertical dragbar to the specified position */
                positionDragY:function(aJ,s){
                    V(aJ,s)
                },
/* Calling the function when trying to animate to a new position. */
...
/* Returning the current co-ordinate of the viewport regarding to the pane */
...
/* Returning the current width of the content within the scroll pane */
                getContentWidth:function(){
                    return T
                },
/* Returning the current height of the content within the scroll pane */
                getContentHeight:function(){
                    return Z
                },
/* Returning the current horizontal position of the viewport within the pane */
                getPercentScrolledX:function(){
                    return aD()/(T-ak)
                },
/* Returning the current vertical position of the viewport within the pane */
                getPercentScrolledY:function(){
                    return aB()/(Z-v)
                },
/* Returning of the scrollpane has a horizontal scrollbar */
                getIsScrollableH:function(){
                    return aF
                },
/* Returning of the scrollpane has a vertical scrollbar */
                getIsScrollableV:function(){
                    return aA
                },
/* Getting a reference to the content pane */
                getContentPane:function(){
                    return Y
                },
/* Scrolling down as far as it can currently do */
                scrollToBottom:function(s){
                    V(i,s)
                },
/* Hijacking links on the page which link to content inside the scrollpane */
                hijackInternalLinks:function(){
                    m()
                },
/* Removing the scrollpane and returning the page to it's state before scrollpane initializing */
                destroy:function(){
                    g()
                }
            });
